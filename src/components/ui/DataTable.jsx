          ...
          <tbody>
            {paginated.length === 0 ? (
              ...
              paginated.map((row, idx) => (
                <tr
                  key={row.clientId || row.id || idx}
                  onClick={() => {
                    console.log('DataTable: Row click detected for row:', row);
                    if (onRowClick) {
                      console.log('DataTable: Invoking onRowClick callback');
                      onRowClick(row);
                    } else {
                      console.warn('DataTable: No onRowClick callback provided');
                    }
                  }}
                >
                  {columns.map(col => (
