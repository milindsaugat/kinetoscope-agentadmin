...
import { getArticleCover } from './GrowDetail';
...
export default function MediaFeed() {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      setLoading(true);
      try {
        const data = await apiRequest('/api/agent/articles').catch(() => apiRequest('/api/client/articles'));
      ...
      {loading ? (
        ...
        <div className="kfpl-media-grid">
          {filtered.map(article => (
            <Link key={article.id} to={`/grow/${article.id}`} className="kfpl-media-card" style={{ textDecoration: 'none', color: 'inherit' }}>
