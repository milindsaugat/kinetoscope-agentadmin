...
export default defineConfig(async ({ mode }) => {
  ...
  const localUrl = env.VITE_API_URL_LOCAL || 'http://192.168.1.22:5000';
  ...
  try {
    ...
    if (res.ok) {
      ...
      console.log('☁️  Local backend responded but unhealthy → using cloud', cloudUrl);
    }
  } catch {
    console.log('☁️  Local backend unavailable → using cloud', cloudUrl);
  }
